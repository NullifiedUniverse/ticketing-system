...
import { getScannerToken, getNgrokUrl, getNetworkConfig, getAlerts } from '../services/api';
...
const Dashboard = () => {
        ...
        const pollAlerts = async () => {
            try {
                ...
                if (connType === 'Checking...') {
                    ...
                    const isNgrok = window.location.hostname.includes('ngrok') || (config.publicUrl && config.publicUrl.includes('ngrok'));
                    setConnType(isNgrok ? 'CLOUD (NGROK)' : 'LOCAL (LAN)');
                    if (config.publicUrl) setPublicUrl(config.publicUrl);
                }
    ...
    }, [eventId, lastAlertTime, showModal, hideModal, connType]);
    ...
    const handleStatusChange = (ticket, action) => {
        // If checking out, maybe different confirm message?
        const title = action === 'check-in' ? t('modalCheckInTitle') : 'Confirm Check-Out'; // todo: add translation
        const body = action === 'check-in' ? t('modalCheckInBody', ticket.attendeeName) : `Are you sure you want to check out ${ticket.attendeeName}?`;

        showConfirmModal(title, body, async () => {
            await handleManualStatusChange(ticket, action);
            hideModal();
        });
    };
    ...
    const generateSetupQR = async () => {
        ...
        try {
            ...
            const publicUrl = netConfig.publicUrl;
            const localUrls = netConfig.localUrls || [];

            // Build Candidates List (Unique, Non-empty)
            // Prioritize Public URL if available for initial reachability, but Scanner will race them.
    ...
    return (
        <Layout>
            <header className="sticky top-2 md:top-6 z-30 mx-3 md:mx-6 rounded-2xl glass-panel px-4 md:px-8 py-3 md:py-5 flex justify-between items-center mb-4 md:mb-8">
                ...
                <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
                    ...
                    {publicUrl && (
                        <div className="hidden xl:flex items-center gap-2 px-3 py-1 rounded-full border bg-emerald-500/10 border-emerald-500/20 text-emerald-400 text-[10px] font-mono tracking-widest cursor-pointer hover:bg-emerald-500/20 transition-colors" onClick={() => { navigator.clipboard.writeText(publicUrl + '/scanner'); alert('Link Copied!'); }} title="Click to Copy Scanner Link">
                    ...
                    {!eventId ? (
                        ...
                        >
                            <motion.div
                                ...
                                animate="visible"
                                whileHover="hover"
                                className="glass-panel p-6 md:p-12 rounded-[2.5rem] max-w-lg w-full text-center border border-white/10 relative overflow-hidden"
                            >
